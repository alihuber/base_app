...
feature "user authentication" do
  ...
  scenario "password reset" do
    ...
    visit base_auth.password_reset_path
    ...
    reset_url =
      base_auth.do_password_reset_path(token: admin_user.password_reset_token)
    ...
    fill_in "user_session_login_password", with: "new_password"
    click_button "submit_login"
    expect(page).to have_css ".alert-success"
  end

  scenario "password reset with wrong token" do
    visit base_auth.do_password_reset_path(token: "wrong")
    fill_in "user_edit_password_reset_password_reset_password",
      with: "new_password"
    click_button "submit_password_reset"

    expect(current_path).to(
      eq base_auth.do_password_reset_path(token: "wrong")
    )
    expect(page).to have_css ".alert-danger"
  end

  scenario "password reset call with blank token" do
    visit base_auth.do_password_reset_path(token: "")

    expect(current_path).to(
      eq base_auth.password_reset_path + "/"
    )
  end
end
