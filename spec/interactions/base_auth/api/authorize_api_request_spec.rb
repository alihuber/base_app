...
module BaseAuth
  ...
  describe Api::AuthorizeApiRequest, type: :model do
    ...
    let!(:jwt_token)   { JsonWebToken.instance.encode(user_id: user.id) }
    let(:jwt_header)   { Hash["headers" => {"Authorization" => jwt_token}] }
    let(:auth_header)  { Hash["headers" => {"Authorization" => user.auth_token}] }
    ...
    describe "validations" do
      subject { described_class.new(jwt_header) }

      it { is_expected.to validate_presence_of :headers }
    end

    describe "#run with jwt params" do
      subject { interaction.run(jwt_header) }
    ...
    describe "#run with token params" do
      subject { interaction.run(auth_header) }
